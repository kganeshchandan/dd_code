...
def newcomputeLigandSimilarity(target_names, fname):
    fingerprints = dict()
    for line in open(fname):
        vals = line.split()
        targ = vals[0]
...
if __name__ == '__main__':
    ...
    if args.cpickle:
        ...
        target_names, targets = get_targets('./og_kiba_data.txt')
	# print("got targets")
        if args.verbose:
...
# 	print("done distance matrix")
        ...
        ligandsim = newcomputeLigandSimilarity(
            target_names, './og_kiba_data.txt')
        # ligandsim = computeLigandSimilarity(target_names, args.pdbfiles)
